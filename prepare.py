...
def main():
    ...
    WRAP_WIDTH = 75  # characters per line in the description column
    ...
    for _, row in df.iterrows():
        ...
        holdout_auc_str = f"{row['holdout_roc_auc']:.4f}" if pd.notna(row.get("holdout_roc_auc")) else "—"
        desc = textwrap.fill(row["description"], width=WRAP_WIDTH)
        line_counts.append(desc.count("\n") + 1)
        table_data.append([
            f"#{int(row['index'])}",
            desc,
            train_auc_str,
    ...
    col_widths = [0.04, 0.44, 0.09, 0.09, 0.09, 0.08, 0.09]
    ...
    # Make every row the same height based on the max number of lines in any description
    max_lines = max(line_counts)
    base_height = table[1, 0].get_height()
    uniform_height = base_height * max_lines
    for i in range(len(line_counts)):
        for j in range(len(col_labels)):
            table[i + 1, j].set_height(uniform_height)

    # Left-align description column (col 1) for all rows
